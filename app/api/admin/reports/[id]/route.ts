...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return adminSuccessResponse(request, { report: { ...report, publicStatus: deriveReportLifecycleStatus(report), createdAt: report.createdAt.toISOString(), updatedAt: report.updatedAt.toISOString(), lastActivityAt: report.lastActivityAt.toISOString(), photoCapturedAt: report.photoCapturedAt?.toISOString() ?? null, resolvedAt: report.resolvedAt?.toISOString() ?? null, archivedAt: report.archivedAt?.toISOString() ?? null } });
}
